...
const ProjectCard: React.FC<ProjectCardProps> = ({
  ...
  return (
    <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 mb-3 flex justify-between items-start">
      ...
      <span
        className={`${tagColor[tag]} text-white text-[9px] px-2 py-1 rounded-lg font-bold flex-shrink-0`}
      >
        {tag}
      </span>
    </div>
...
const Home: React.FC = () => {
  return (
    <div className="flex justify-center min-h-screen bg-[#F8FAFC] font-sans">
      <div className="bg-gray-50 min-h-screen shadow-2xl relative pb-24 overflow-y-auto">
        {/* Header Section */}
        ...
        <section className="px-5 mt-8">
          ...
          <div className="bg-white rounded-2xl p-2 shadow-sm border border-gray-100">
            {[1, 2, 3].map((_, i) => (
              <div
                key={i}
                className="p-3 border-b last:border-none flex justify-between items-center hover:bg-gray-50 transition-colors"
              >
                <div>
        ...
        <nav className="absolute bottom-0 w-full bg-white border-t border-gray-100 px-6 py-3 flex justify-between items-center shadow-[0_-5px_20px_rgba(0,0,0,0.05)] rounded-t-[20px]">
          {["홈", "팀원찾기", "프로젝트", "게시판", "마이"].map((menu, idx) => (
            <button
              key={menu}
              className={`flex flex-col items-center gap-1 transition-colors ${idx === 0 ? "text-[#5C7CFF]" : "text-gray-400 hover:text-gray-600"}`}
            >
