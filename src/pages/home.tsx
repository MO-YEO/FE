  ...
  return (
    <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 mb-3 flex justify-between items-start">
      <div>
        ...
        <p className="text-[11px] text-gray-500 mb-2">작성자: {author}</p>
        <div className="flex items-center gap-3 text-gray-400 text-[10px]">
          <span>👥 {members}명</span>
          <span>🕒 {time}</span>
        </div>
      </div>
      <span className={`${tagColor[tag]} text-white text-[9px] px-2 py-1 rounded-lg font-bold flex-shrink-0`}>
        {tag}
      </span>
    </div>
  );
...
const Home: React.FC = () => {
  return (
    <div className="flex justify-center min-h-screen bg-[#F8FAFC] font-sans">
      
      <div className="w-[382px] bg-gray-50 min-h-screen shadow-2xl relative pb-24 overflow-y-auto">
        
        {/* Header Section */}
        <header className="bg-[#5C7CFF] pt-12 pb-10 px-6 rounded-b-[40px] shadow-lg">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-white text-2xl font-black tracking-tighter italic">MO-YEO</h1>
          </div>
        ...
        <section className="grid grid-cols-2 gap-4 px-5 -mt-6">
          <button className="bg-[#6B8BFF] p-5 rounded-3xl text-white shadow-xl flex flex-col items-start transition-transform active:scale-95">
            <div className="bg-white/20 p-2 rounded-lg mb-3 text-lg">👥</div>
        ...
        <section className="px-5 mt-8">
          <div className="flex justify-between items-center mb-4">
            <h2 className="font-bold text-gray-800 flex items-center gap-1">
              <span className="text-lg">📈</span> 최근 프로젝트
            </h2>
            <button className="text-xs text-blue-500 font-semibold">전체 →</button>
          </div>
        ...
        <section className="px-5 mt-8">
          <div className="flex justify-between items-center mb-4">
            <h2 className="font-bold text-gray-800 flex items-center gap-1">
              <span className="text-lg">💬</span> 최근 게시글
            </h2>
            <button className="text-xs text-blue-500 font-semibold">전체 →</button>
          </div>
          <div className="bg-white rounded-2xl p-2 shadow-sm border border-gray-100">
            {[1, 2, 3].map((_, i) => (
              <div key={i} className="p-3 border-b last:border-none flex justify-between items-center hover:bg-gray-50 transition-colors">
                <div>
        ...
        <nav className="absolute bottom-0 w-full bg-white border-t border-gray-100 px-6 py-3 flex justify-between items-center shadow-[0_-5px_20px_rgba(0,0,0,0.05)] rounded-t-[20px]">
          {['홈', '팀원찾기', '프로젝트', '게시판', '마이'].map((menu, idx) => (
            <button key={menu} className={`flex flex-col items-center gap-1 transition-colors ${idx === 0 ? 'text-[#5C7CFF]' : 'text-gray-400 hover:text-gray-600'}`}>
