import { useState, useEffect } from "react";
import FieldLabel from "../components/fieldLabel";
import { useNavigate } from "react-router-dom";
import { PATH } from "../components/path";
import Input from "../components/input";
import Textarea from "../components/textarea";
import { membersApi } from "../api/member";

const SignUpPage = () => {
  const [tags, setTags] = useState<string[]>([]);
  const [input, setInput] = useState("");
  const [isPageLoading, setIsPageLoading] = useState(true);
  const navigate = useNavigate();

  useEffect(() => {
    const checkUser = async () => {
      try {
        await membersApi.getMyProfile();
      } catch (e) {
        console.error("기존 회원 검증 실패:", e);
      } finally {
        setIsPageLoading(false);
      }
    };
    checkUser();
  }, [navigate]);

  const handleAddTag = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter" && input.trim() !== "") {
      e.preventDefault();
      setTags([...tags, input.trim()]);
      setInput("");
    }
  };

  const handleDelete = (deleteTag: string) => {
    setTags((prev) => prev.filter((tag) => tag !== deleteTag));
  };

  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const formData = new FormData(e.currentTarget);
    const data = Object.fromEntries(formData.entries());

    const finalData = {
      nickname: String(data.nickname),
      profileImageUrl: "",
      role: String(data.role),
      contactEmail: String(data.email),
      phoneNumber: "",
      githubUrl: String(data.githubUrl),
      intro: String(data.intro),
      techStacks: tags,
      activityCategories: []
    };

    try {
      await membersApi.updateMyProfile(finalData);
      alert("계정 생성이 완료되었습니다!");
      navigate(PATH.HOME, { replace: true });
    } catch (error) {
      console.error("등록 실패:", error);
      alert("등록 실패 (백엔드 준비 중일 수 있습니다)");
    }
  };

  if (isPageLoading) {
    return (
      <div className="flex h-screen items-center justify-center bg-[#F7F8FA]">
        <div className="w-10 h-10 border-4 border-[#2F6BFF] border-t-transparent rounded-full animate-spin"></div>
      </div>
    );
  }

  return (
    <main className="mx-auto min-h-screen w-full max-w-[400px] bg-[#F7F8FA] pb-[100px] relative text-left">
      <div className="px-5 py-10 flex flex-col gap-8">
        <div className="flex flex-col gap-2">
          <p className="font-bold text-[24px]">계정 생성</p>
          <p className="text-[14px] text-[#64748B]">프로필 정보를 입력해주세요</p>
        </div>

        <form id="signUpForm" onSubmit={handleSubmit} className="flex flex-col gap-6">
          <div className="flex flex-col gap-2">
            <FieldLabel label="이름" required={true} />
            <Input name="nickname" placeholder="이름을 입력하세요" />
          </div>

          <div className="flex flex-col gap-2">
            <FieldLabel label="직무" required={true} />
            <Input name="role" placeholder="예: 프론트엔드 개발자" />
          </div>

          <div className="flex flex-col gap-2">
            <FieldLabel label="이메일" required={true} />
            <Input name="email" placeholder="example@email.com" />
          </div>

          <div className="flex flex-col gap-2">
            <FieldLabel label="깃허브 주소" required={false} />
            <Input name="githubUrl" placeholder="https://github.com/username" />
          </div>

          <div className="flex flex-col gap-2">
            <FieldLabel label="자기소개" required={true} />
            <Textarea name="intro" placeholder="자기소개를 입력하세요" />
          </div>

          <div className="flex flex-col gap-2">
            <FieldLabel label="기술 스택" required={true} />
            <Input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={handleAddTag} placeholder="입력 후 Enter" />
            <div className="flex gap-2 flex-wrap mt-1">
              {tags.map((tag) => (
                <span key={tag} className="text-[#0069A8] bg-[#F0F9FF] px-3 py-1 rounded-full text-[12px] font-medium border border-[#B3E0FF]">
                  {tag} <button type="button" onClick={() => handleDelete(tag)}>×</button>
                </span>
              ))}
            </div>
          </div>
        </form>
      </div>

      <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[400px] p-5 bg-white border-t border-gray-100 z-50">
        <button type="submit" form="signUpForm" className="bg-[#2F6BFF] w-full h-14 rounded-[14px] text-white font-bold active:scale-95 transition-all">
          계정 생성 완료
        </button>
      </div>
    </main>
  );
};

export default SignUpPage;